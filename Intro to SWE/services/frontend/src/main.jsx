...
function App() {
  ...
  const createOrder = async () => {
    ...
    try {
      ...
      showMessage(`Order #${data.id} created - proceed to payment`, 'success');
      setShowPayment(true);
    } catch (err) {
  ...
  const handlePaymentSuccess = async (paymentData) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URLS.orders}/orders/${currentOrder.id}/complete`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${getToken()}`
  ...
  const handlePaymentCancel = async () => {
    ...
    try {
      // Cancel the order
      const res = await fetch(`${API_URLS.orders}/orders/${currentOrder.id}/cancel`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${getToken()}`
      ...
      if (res.ok) {
        ...
        showMessage('Payment cancelled - order has been cancelled', 'info');
      } else {
