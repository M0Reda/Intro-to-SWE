...
function App() {
  ...
  const createOrder = async () => {
    ...
    // Don't create order in database yet - just prepare data for payment
  ...
  const handlePaymentSuccess = async (paymentData) => {
    ...
    try {
      ...
      // Complete the order - this will update status and deduct inventory
      ...
      showMessage('Payment completed successfully! Order confirmed and inventory updated.', 'success');
      setShowPayment(false);
      setCurrentOrder(null);
      await clearCart();
      
      // Redirect to appropriate orders tab
      setActiveTab(isAdmin ? 'myorders' : 'orders');

    } catch (err) {
      console.error('Failed to complete order:', err);
      showMessage(err.message || 'Order completion failed', 'error');
      // Keep payment modal open so user can try again or cancel
    } finally {
      setLoadingState(false);
    }
  };
  ...
  const handleCODPayment = async () => {
    ...
    try {
      ...
      // Confirm COD order and deduct inventory (but keep status as pending)
      ...
      showMessage('Order placed successfully! Inventory reserved. Payment will be collected on delivery. Status: PENDING', 'success');
      setShowPayment(false);
      setCurrentOrder(null);
      await clearCart();
  ...
  const handlePaymentCancel = async () => {
    // Just close the payment modal - no order was created
    setShowPayment(false);
    setCurrentOrder(null);
