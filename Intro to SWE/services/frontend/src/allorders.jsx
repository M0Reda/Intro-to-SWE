...
function AllOrders({ apiUrl = 'http://localhost:3000' }) {
  ...
  const filteredOrders = orders.filter(order => {
    ...
    const matchesUser = !searchUserId || order.user_id.toLowerCase().includes(searchUserId.toLowerCase());
            ...
            <div key={order.id} className="order-card">
              <div className="order-header" onClick={() => toggleOrder(order.id)}>
                <div className="order-header-left">
                  ...
                  <span className="order-user-id">👤 User: {order.user_id.substring(0, 20)}...</span>
                </div>
