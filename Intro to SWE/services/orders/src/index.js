...
console.log('✅ COD (Cash on Delivery) support enabled');
console.log('✅ Order cancellation with inventory restoration enabled');
...
// POST /orders/:id/confirm-cod - Confirm COD order and deduct inventory (status stays pending)
app.post('/orders/:id/confirm-cod', authenticate, async (req, res) => {
  ...
  try {
    ...
    // Get the order - admin can confirm any order
    let orderResult;
    if (username === 'admin') {
    ...
    for (const item of order.items) {
      ...
      if (inventoryResult.rows.length === 0) {
        ...
        return res.status(400).json({ 
          error: `Insufficient stock for ${item.sku}. Order cannot be confirmed.`,
          sku: item.sku
        });
      }

      console.log(`✅ Deducted ${item.qty} units of ${item.sku} from inventory (COD)`);
    }

    // Keep status as pending (payment will be collected on delivery)
    // Order is confirmed and inventory is reserved, but payment not yet received
    await client.query('COMMIT');

    console.log(`✅ COD Order ${orderId} confirmed and inventory deducted (status: pending)`);
    res.json({
      order: order,
      message: 'COD order confirmed and inventory reserved. Status remains pending until payment collected.'
    });
...
// POST /orders/:id/complete - Mark order as completed and deduct inventory (for paid orders)
...
// POST /orders/:id/cancel - Cancel an order and restore inventory
app.post('/orders/:id/cancel', authenticate, async (req, res) => {
  ...
  try {
    ...
    // Get the order - admin can cancel any order
    let orderResult;
    if (username === 'admin') {
      orderResult = await client.query(
    ...
    // Only restore inventory if order was completed or pending (inventory was deducted)
    // For COD confirmed orders and completed orders, we need to restore inventory
    if (order.status === 'completed' || order.status === 'pending') {
      ...
      for (const item of order.items) {
        ...
        console.log(`✅ Restored ${item.qty} units of ${item.sku} to inventory`);
      }
    }
    ...
    console.log(`✅ Order ${orderId} cancelled by ${username} - inventory restored`);
    res.json({
      order: updateResult.rows[0],
...
const PORT = 3000;
app.listen(PORT, () => console.log(`📦 Orders service running on port ${PORT} (JWT DECODE AUTH + ADMIN + COD + CANCEL)`));