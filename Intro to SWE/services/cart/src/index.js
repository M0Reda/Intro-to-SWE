...
app.get('/cart', keycloak.middleware(), async (req, res) => {
  try {
...
app.get('/cart/:userId', keycloak.middleware(), async (req, res) => {
  try {
    const userId = req.user.sub || req.user.preferred_username;
    const result = await pool.query('SELECT * FROM cart WHERE user_id = $1', [userId]);
...
app.post('/cart', keycloak.middleware(), async (req, res) => {
  try {
    const userId = req.user.sub || req.user.preferred_username;
...
app.post('/cart/:userId', keycloak.middleware(), async (req, res) => {
  try {
    const userId = req.user.sub || req.user.preferred_username;
    const { sku, qty, name, price } = req.body;
...
app.put('/cart/:sku', keycloak.middleware(), async (req, res) => {
  try {
    const userId = req.user.sub || req.user.preferred_username;
...
// Legacy endpoint - PROTECTED
app.put('/cart/:userId/:sku', keycloak.middleware(), async (req, res) => {
  try {
    const userId = req.user.sub || req.user.preferred_username;
    const { sku } = req.params;
    const { qty } = req.body;
...
app.delete('/cart/:sku', keycloak.middleware(), async (req, res) => {
  try {
    const userId = req.user.sub || req.user.preferred_username;
    const { sku } = req.params;
...
// Legacy endpoint - PROTECTED
app.delete('/cart/:userId/:sku', keycloak.middleware(), async (req, res) => {
  try {
    const userId = req.user.sub || req.user.preferred_username;
    const { sku } = req.params;
...
app.delete('/cart', keycloak.middleware(), async (req, res) => {
  try {
    const userId = req.user.sub || req.user.preferred_username;
...
// Legacy endpoint - PROTECTED
app.delete('/cart/:userId', keycloak.middleware(), async (req, res) => {
  try {
    const userId = req.user.sub || req.user.preferred_username;
    await pool.query('DELETE FROM cart WHERE user_id = $1', [userId]);
    res.json({ userId, items: [] });
